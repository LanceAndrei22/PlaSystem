...
public class DataHandling extends ProductData {
    ...
    public void editSelectedData(LinkedList<ProductData> list, JTable table, int selectedRow, int newQuantity, double newPrice, String newName, String newSize, String newBrand, String newType, int newRestockvalue) {
        ...
        if (selectedRow >= 0 && selectedRow < list.size()) {
            ...
            model.setValueAt(newRestockvalue, selectedRow, 7);  // Update publisher

            // Update the data in the linked list
            ProductData editedProductData = list.get(selectedRow);         // Get the selected ProductData object
            editedProductData.setProductQuantity(newQuantity);             // Update quantity
            editedProductData.setProductPrice(newPrice);                  // Update price
            editedProductData.setProductName(newName);                   // Update name
            editedProductData.setProductSize(newSize);                  // Update size
            editedProductData.setProductBrand(newBrand);                // Update brand
            editedProductData.setProductType(newType);                // Update type
            editedProductData.setProductRestockvalue(newRestockvalue);                // Update Restockvalue
